...
function input_data(box,secretkey,outfile;resolution::Float64=0.0015, basinlvl::Int=7, ascending::Bool=true)

   session = EESession(secretkey)
   Initialize(session)

   box = Rectangle(bbox...);
   basin_geo = geometry(box)
   ...
   scenes = filter(
      filterBounds(EE.ImageCollection("COPERNICUS/S1_GRD"), basin_geo),
      And(EE.Filter(),
...
function parse_commandline()
    ...
    @add_arg_table s begin
        ...
        nargs = 4
        arg_type = Float64
        help = "bounding box to extract data for"
        required = true
